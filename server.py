...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "Use run_command for quick commands (<60s). "
        "Use start_job for long-running commands such as "
        "video renders, installs, downloads, builds and model "
        "downloads. Poll with job_status and job_output. "
        "Use download_file to obtain a direct URL for workspace files."
    ),
)
...
def run_command(
    ...
    try:
        ...
        return (
            ...
            f"{timeout_seconds}s. "
            f"Use start_job for long-running commands."
        )
...
def kill_job(
    ...
    if not pid:
        ...
        return (
            f"Job {job_id} has no recorded pid"
        )
...
def search_files(
    ...
    """Search workspace contents with regex."""
...
def file_info(
    ...
    """Return information about a workspace file."""
...
def download_file(
    ...
    """
    Return a direct URL for a workspace file.
...
#
# IMPORTANT:
# This is registered ON THE MCP SERVER.
#
# We do NOT Mount /mcp ourselves.
#
# Therefore streamable_http_app() remains responsible
...
# These routes are added to the MCP application's own
# Starlette application.
#
# The MCP SDK supports custom HTTP routes alongside /mcp.
...
if __name__ == "__main__":
    ...
    # Same configuration as your ORIGINAL server.
    # No authentication.
    # No token.
    # No secrets.
    # No extra variables.
    ...
    # DO NOT Mount this at /mcp.
    #
    # streamable_http_app() already exposes /mcp.
